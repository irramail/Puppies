...
class PuppyTreeDatabaseModel(db: Database)(implicit ec: ExecutionContext) {
  ...
  def getPuppies(username: String): Future[Seq[PuppyItem]] = {
    db.run(
      (for {
    ...
    ).map(items => items.map(item => PuppyItem(item.itemId, item.text)))
  }

  def addPuppy(userid: Int, task: String): Future[Int] = {
    db.run(Items += ItemsRow(-1, userid, task))
  }

  def removePuppy(itemId: Int): Future[Boolean] = {
